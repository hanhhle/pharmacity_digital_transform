  ...
  <div id="quickview-modal-backdrop" onclick="if(event.target === this) closeQuickViewModal()" class="fixed inset-0 bg-slate-900/60 z-50 backdrop-blur-xs flex items-center justify-center p-4 hidden transition-opacity duration-200">
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-[840px] overflow-hidden relative border border-slate-100 flex flex-col md:flex-row max-h-[90vh]">
      ...
      <div class="flex-1 p-6 flex flex-col justify-between space-y-4 overflow-y-auto">
        ...
        <!-- Action Buttons Form -->
        <form action="/index.php?route=checkout" method="POST" class="pt-3 border-t border-slate-100">
          ...
          <div class="flex items-center gap-3">
            <button type="submit" name="action" value="add_to_cart" class="flex-1 font-bold rounded-xl border border-blue-600 bg-white text-blue-600 hover:bg-blue-50 py-3 text-xs md:text-sm transition-all shadow-2xs">
              Thêm vào giỏ
            </button>
            <button type="submit" name="action" value="buy_now" class="flex-1 font-bold rounded-xl bg-blue-600 hover:bg-blue-700 text-white py-3 text-xs md:text-sm transition-all shadow-md">
              Mua ngay
            </button>
          </div>
        </form>
